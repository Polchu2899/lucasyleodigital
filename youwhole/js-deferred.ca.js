...
function initChat(){
  ...
  function respuestaLocal(text){
    ...
    if(msg.includes('verifactu')){
      return 'VeriFactu és el nou sistema de l\'Agència Tributària per verificar factures en temps real 🧾. YouWhole genera el hash encadenat, la signatura XAdES i el QR de cada factura, i l\'envia a l\'AEAT automàticament. No has de fer res manualment.';
    }else if(msg.includes('preu')||msg.includes('costa')){
      return 'YouWhole té 4 plans: **Free** (0€), **Starter** (29€/mes), **Pro** (79€/mes, el més triat) i **Enterprise** (199€/mes) 💳. Pots començar gratis i pujar de pla quan ho necessitis, sense permanència.';
    }else if(msg.includes('autònom')||msg.includes('autonom')){
      return 'Sí, YouWhole està pensat també per a autònoms espanyols 🇪🇸. Calcula automàticament l\'IRPF a les teves factures i genera el Model 130 trimestral amb les deduccions prèvies ja aplicades.';
    }else if(msg.includes('alta')||msg.includes('registr')||msg.includes('començ')||msg.includes('provar')){
      return 'Donar-te d\'alta és gratis i triga menys de 2 minuts 🚀. Prem a "Provar Gratis" o "Començar Gratis Ara" en aquesta mateixa pàgina i crea el teu compte directament.';
    }else if(msg.includes('diferència')||msg.includes('per què youwhole')){
      return 'YouWhole neix amb VeriFactu integrat des del primer dia, i inclou control horari amb GPS i portal de client sense mòduls de pagament addicionals 💡. Tot pensat perquè gestionis el teu negoci sense fricció.';
    }else if(msg.includes('idioma')||msg.includes('divisa')||msg.includes('internacional')){
      return 'YouWhole funciona en 5 idiomes (espanyol, català, èuscar, gallec i anglès) i suporta més de 20 divises amb tipus de canvi del BCE 🌍. Ideal si factures també fora d\'Espanya.';
    }else if(msg.includes('horari')||msg.includes('fitxatge')||msg.includes('emplea')){
      return 'El mòdul de control horari permet fitxar entrada/sortida amb GPS, fitxar per QR, veure hores extra automàtiques i gestionar vacances des d\'un portal d\'empleat 🕒.';
    }else if(msg.includes('contact')||msg.includes('parlar')||msg.includes('trucar')||msg.includes('whatsapp')){
      return 'Ens pots escriure a info@lucasyleodigital.com o per WhatsApp des del botó verd d\'aquesta pàgina 📞. Et responem en menys de 24h.';
    }else if(msg.includes('hola')||msg.includes('bones')||msg.includes('hey')){
      return 'Hola! 👋 En què et puc ajudar? Et puc explicar sobre VeriFactu, preus, funcionalitats o com donar-te d\'alta a YouWhole.';
    }else{
      return 'Bona pregunta! Per a una resposta més concreta, el millor és escriure\'ns a info@lucasyleodigital.com o per WhatsApp 🎯. També pots explorar les seccions d\'aquesta pàgina: Funcionalitats, VeriFactu i Preus.';
    }
  }
