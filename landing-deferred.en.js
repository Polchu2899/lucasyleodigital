...
function initChat() {
  ...
  function respuesta(text) {
    ...
    if (m.includes('session') || m.includes('free') || m.includes('book') || m.includes('call'))
      return 'The strategy session is a 45-minute video call where we analyze your digital situation together. I identify what you have, what\'s missing and what steps to take. No commitment — if I can\'t help you, I\'ll tell you. You can book your slot right here on the site, in the calendar further down.';

    if (m.includes('price') || m.includes('cost') || m.includes('rate') || m.includes('budget'))
      return 'Pricing depends on what you need. I work with consulting sessions, tailored projects and monthly support. The best thing is to book the free session — that\'s where I understand your situation and give you a clear proposal. No surprises.';

    if (m.includes('how do you work') || m.includes('method') || m.includes('approach') || m.includes('process'))
      return 'My approach has 3 phases: first **Clarity** (understanding your real situation), then **System** (designing the right channels connected to each other) and finally **Conversion** (activating, measuring and adjusting so the system works for you). No detours or isolated actions.';

    if (m.includes('service') || m.includes('help') || m.includes('offer'))
      return 'I offer three ways to work together: **Strategic diagnosis** (free 45-min session), **Digital system creation** (website, advertising, integrated automations) and **Ongoing support** (we review metrics and adjust strategy every month). Which one fits you best?';

    if (m.includes('business') || m.includes('type of') || m.includes('who') || m.includes('for whom'))
      return 'I work with small and medium businesses that already have clients and want to grow in an organized way. Hospitality, professional services, health, training, retail... If you have a real business and want a digital system that works, let\'s talk.';

    if (m.includes('barcelona') || m.includes('location') || m.includes('where') || m.includes('city') || m.includes('in person'))
      return 'I\'m based in Barcelona, but I work with businesses across Spain 100% online. I handle most of my clients by video call without any issue.';

    if (m.includes('result') || m.includes('how long') || m.includes('works'))
      return 'It depends on your starting point. Some improvements show up in weeks (advertising, web conversion) while others take months (rankings, brand authority). What I don\'t do is promise miracle results — I\'m honest about timelines.';

    if (m.includes('website') || m.includes('web page') || m.includes('landing'))
      return 'I build conversion websites: designed so the visitor understands your offer and takes action (book, call, buy). No decorative websites that don\'t generate clients.';

    if (m.includes('advertis') || m.includes('ads') || m.includes('google') || m.includes('meta') || m.includes('facebook') || m.includes('instagram'))
      return 'I manage digital advertising campaigns (Google Ads, Meta Ads) with a focus on real ROI. It\'s not just about running ads — it\'s about designing an acquisition system that works month after month.';

    if (m.includes('automat') || m.includes('ai') || m.includes('artificial intelligence') || m.includes('bot'))
      return 'I implement AI automations so your business runs without you being on top of everything: automatic replies, lead follow-up, appointment management... The goal is for the system to work for you.';

    if (m.includes('agency'))
      return 'Besides my work as an advisor, I also run a full agency with more services. You can check it out at lucasyleodigital.com/en/agencia. But if you\'re looking for a close, personalized relationship, you\'re in the right place.';

    if (m.includes('contact') || m.includes('whatsapp') || m.includes('call') || m.includes('email'))
      return 'You can message me on WhatsApp at +34 624 029 617, or book your free session directly in the calendar on this page. I\'ll reply within 24h.';

    if (m.includes('hello') || m.includes('hi') || m.includes('hey'))
      return 'Hi! Glad you\'re here. What can I help you with? I can tell you about the free session, my services, or how I work with businesses like yours.';

    if (m.includes('thank') || m.includes('great') || m.includes('perfect'))
      return 'You\'re welcome! If you need anything else, I\'m here. And if you want to take the next step, book your free session in the calendar — no commitment.';

    return 'Good question. For the best answer about your specific case, the ideal is to book the free 45-min session. There we analyze your situation and I give you a clear plan. You can also message me on WhatsApp at +34 624 029 617.';
  }
...
function initCalendar() {
  ...
  (async function() {
    ...
    window.resySelDia = function(fecha, d, btn, isSab) {
      ...
      document.getElementById('resy-fecha-lbl').textContent = MESES[cM] + ' ' + d + ', ' + cY;
    ...
    window.resySelSlot = function(hora, tipo, btn) {
      selFranja = hora + ' (' + (tipo === 'lv' ? 'Mon-Fri' : tipo === 'sabm' ? 'Sat morning' : 'Sat afternoon') + ')';
      document.querySelectorAll('.resy-slot').forEach(function(b) { b.classList.remove('rs-sel'); });
      btn.classList.add('rs-sel');
      var p = selDate.split('-');
      document.getElementById('resy-ok-txt').textContent = MESES[parseInt(p[1])-1] + ' ' + parseInt(p[2]) + ', ' + p[0] + ' · ' + hora + 'h';
    ...
    document.getElementById('resyForm').addEventListener('submit', async function(e) {
      ...
      if (!nombre) { showRNotif('⚠️ Please enter your name.', 'error'); return; }
      if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showRNotif('⚠️ Please enter a valid email.', 'error'); return; }
      if (!selDate) { showRNotif('⚠️ Please pick a day in the calendar.', 'error'); return; }
      if (!selFranja) { showRNotif('⚠️ Please select a time slot.', 'error'); return; }
      if (!rgpd) { showRNotif('⚠️ Please accept the Privacy Policy.', 'error'); return; }
      ...
      try {
        ...
        try {
          await fetch(APPS + '?' + new URLSearchParams({
            ...
            origin: 'Landing Advisor Lucas EN — Calendar'
        ...
        gNotif.textContent = '✅ Request sent! I\'ll confirm the exact time within 24h.';
        ...
        gNotif2.textContent = '✅ Request received! We\'ll contact you within 24h.';
        ...
        btn.textContent = 'Request my free session →';
        btn.disabled = false;
      }
    });
